...
// ---------------------------------------------------------------------------
// LiquidEther — interactive fluid-dynamics hero background
//
// Drop-in usage:
//   <div style={{ position: 'relative', width: '100%', height: '100vh' }}>
//     <LiquidEther colors={['#5227FF', '#FF9FFC', '#B19EEF']} autoDemo />
//     <div style={{ position: 'relative', zIndex: 1 }}>…your content…</div>
//   </div>
//
// All props have sensible defaults so the component works with zero config.
// ---------------------------------------------------------------------------
...
export default function LiquidEther({
  ...
  // Auto-demo (runs when the user is idle / cursor is outside)
  autoDemo = true,
  autoSpeed = 0.5,
  autoIntensity = 2.2,
  ...
  // ---------------------------------------------------------------------------
  // Setup effect — only recreates the WebGL instance when `colors` changes,
  // because that requires rebuilding the palette texture from scratch.
  // All other props are updated live by the second effect below.
  // ---------------------------------------------------------------------------
  useEffect(() => {
    ...
    class Output {
      constructor(simOptions) {
        this.scene = new THREE.Scene();
        this.camera = new THREE.Camera();
        // Simulation receives the real options from the start — no post-hoc patch needed
    ...
    class WebGLManager {
      constructor(props) {
        ...
        // Pass simulation options so FBOs are sized correctly from the first frame
        this.output = new Output({
          mouse_force: props.mouse_force,
    ...
    const webgl = new WebGLManager({
      $wrapper: container,
      // Simulation options (used for correct initial FBO sizing + first frame)
  ...
  }, [colors]); // only rebuild when the color palette changes

  // ---------------------------------------------------------------------------
  // Live-update effect — updates simulation/auto-driver options every time a
  // non-color prop changes, without tearing down the WebGL instance.
  // ---------------------------------------------------------------------------
  useEffect(() => {
    ...
    const ad = webgl.autoDriver;
    if (ad) {
      ad.enabled = autoDemo;
      ad.speed = autoSpeed;
      ad.resumeDelay = autoResumeDelay;
      ad.rampDurationMs = autoRampDuration * 1000;
      if (ad.mouse) {
        ad.mouse.autoIntensity = autoIntensity;
        ad.mouse.takeoverDuration = takeoverDuration;
      }
    }
  }, [
    mouseForce,
